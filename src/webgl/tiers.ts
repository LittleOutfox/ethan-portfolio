...
/** a software rasterizer (a VM, remote desktop, a broken driver) never runs the field */
...
export function detectTier(): TierSpec | null {
  ...
  if (cores <= 8 || mem < 8 || /Intel/i.test(renderer) || MID_GPU.test(renderer)) return SPECS.medium
  return SPECS.high
}
